...
object HigherOrderFunctions extends App {
  ...
  // Scan Right - right associative, return entire result list
  PrintUtils.printHeader("Scan Right Example")
  val someNumbers = List(10, 20, 30, 40, 50, 60)
  val someNumbersScanRight = someNumbers.scanRight(0)(_ - _)
  someNumbersScanRight.foreach(x => print(x + " "))
  
  // Scan Left -left associative, return entire result list

  // Fold -
  // foldLeft - return only the last element after the last execution of scan left operation

  // Reduce
  // reduceLeft - No initial value, first two list elments as operands in the first
  // reduceRight
